...
class VideoRepositoryTest extends RepositoryTest {
    ...
    @TestFactory
    @DisplayName("page, sort, category 로 video 를 조회한다.")
    Collection<DynamicTest> findByCategory() {
        ...
        return List.of(
            dynamicTest("category java 로 조회하면 video 4, 2, 1 이 조회된다.", ()-> {
                //when
                Page<Video> videos = videoRepository.findAllByCategoryPaging("java", pageRequest, null);
            ...
            dynamicTest("category spring 로 조회하면 video 5, 3, 1 이 조회된다.", ()-> {
                //when
                Page<Video> videos = videoRepository.findAllByCategoryPaging("spring", pageRequest, null);
            ...
            dynamicTest("별점 순으로 조회하면 video3 이 먼저 조회된 후 나머지는 최신순으로 조회된다.", ()-> {
                //when
                Page<Video> videos = videoRepository.findAllByCategoryPaging(null, pageRequest, "star");
                ...
                assertThat(videos.getContent().get(3).getVideoId()).isEqualTo(video2.getVideoId());
                assertThat(videos.getContent().get(4).getVideoId()).isEqualTo(video1.getVideoId());
            }),
            dynamicTest("조회 순으로 조회하면 video2 가 먼저 조회된 후 나머지는 최신순으로 조회된다.", ()-> {
                //when
                Page<Video> videos = videoRepository.findAllByCategoryPaging(null, pageRequest, "view");
            ...
            dynamicTest("최신순으로 조회하면 video5 가 먼저 조회된다.", ()-> {
                //when
                Page<Video> videos = videoRepository.findAllByCategoryPaging(null, pageRequest, null);

                //then
                assertThat(videos.getContent().get(0).getVideoId()).isEqualTo(video5.getVideoId());
                assertThat(videos.getContent().get(1).getVideoId()).isEqualTo(video4.getVideoId());
                assertThat(videos.getContent().get(2).getVideoId()).isEqualTo(video3.getVideoId());
                assertThat(videos.getContent().get(3).getVideoId()).isEqualTo(video2.getVideoId());
                assertThat(videos.getContent().get(4).getVideoId()).isEqualTo(video1.getVideoId());
            }),
            dynamicTest("별점 순, spring 으로 조회하면 video3, video5, video1 순서로 조회된다.", ()-> {
                //when
                Page<Video> videos = videoRepository.findAllByCategoryPaging("spring", pageRequest, "star");
            ...
            dynamicTest("조회 순, java 로 조회하면 video2, video4, video1 순서로 조회된다.", ()-> {
                //when
                Page<Video> videos = videoRepository.findAllByCategoryPaging("java", pageRequest, "view");
