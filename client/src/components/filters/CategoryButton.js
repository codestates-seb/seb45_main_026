...
export default function CategoryButton({ filter }) {
  ...
  const filterState = useSelector(
    (state) => state.filterSlice.filter[filter.name]
  );
  ...
  useEffect(() => {
    const dropdownHandler = (e) => {
      if (
        obj[`${filter.name}Ref`].current !== null &&
        !obj[`${filter.name}Ref`].current.contains(e.target)
      ) {
  ...
  return (
    <ButtonContainer>
      ...
      >
        {filterState.value !== filter.initialValue
          ? filterState.text
          : filter.initialText}
      </FilterButton>
