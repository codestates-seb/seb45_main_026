...
export default function CategoryFilter({ filterNum }) {
  ...
  useEffect(() => {
    axios.get("https://api.itprometheus.net/categories").then((res) => {
      const categoryData = res.data.data.map((el) => ({
  ...
  return (
    <FilterContainer>
      {filters[filterNum].map((el, idx) => (
        <CategoryButton key={idx} filter={el} />
      ))}
    </FilterContainer>
  );
}
