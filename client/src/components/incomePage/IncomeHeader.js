...
export const IncomeAmountHeadTypo = styled(IncomeAmountTypo)`
  ...
  color: ${(props) =>
    props.isDark ? globalTokens.LightGray.value : globalTokens.Gray.value};
`;
export const IncomeIndexHeadTypo = styled(IncomeTitleTypo)`
  width: 70px;
  color: ${(props) =>
    props.isDark ? globalTokens.LightGray.value : globalTokens.Gray.value};
`;
...
const IncomeHeader = () => {
  ...
  return (
    <IncomeHeadContainer isDark={isDark}>
      <IncomeIndexHeadTypo isDark={isDark}>ID</IncomeIndexHeadTypo>
      <IncomeTitleHeadContainer isDark={isDark}>
        강의명
      </IncomeTitleHeadContainer>
      <IncomeAmountHeadTypo isDark={isDark}>
        누적 판매 금액
