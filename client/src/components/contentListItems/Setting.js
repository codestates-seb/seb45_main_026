...
const Setting = () => {
  ...
  const [isDeleteUserConfirmModalOpen, setIsDeleteUserConfirmModalOpen] =
    useState(false);
  const [isDeleteUserAlertModalOpen, setIsDeleteUserAlertModalOpen] =
    useState(false);
  ...
  const handleAccountNumberClick = async () => {
    ...
    if (isAccountHolder && bank && isAccountNumber) {
      ...
      <SettingContainer isDark={isDark}>
        <UserInfoContainer>
          ...
          {errors.channelDescription &&
            errors.channelDescription.type === "required" && (
              <InputErrorTypo isDark={isDark} width="50vw">
                채널 설명을 입력해 주세요.
              </InputErrorTypo>
            )}
          ...
          <ExtraButtonContainer>
            <a href="https://field-hellebore-58d.notion.site/c50fadcd93e04c4fb19580c0f99ca773?pvs=4">
              <PositiveTextButton isDark={isDark} type="button">
                이용약관 보기
              </PositiveTextButton>
            </a>
...
export const BankLists = styled.ul`
  width: 50vw;
  max-width: 900px;
...
export const BankList = styled.li`
  margin: 5px 5px;
  padding: 10px 10px;
  border-radius: ${globalTokens.RegularRadius.value}px;
  border: ${globalTokens.ThinHeight.value}px solid
    ${(props) => (props.isSelected ? "yellow" : globalTokens.LightGray.value)};
  background-color: ${(props) => (props.isSelected ? "yellow" : "")};
  color: ${(props) =>
    props.isDark ? globalTokens.White.value : globalTokens.Black.value};
