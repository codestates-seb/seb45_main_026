...
const ListBody = styled.div`
  ...
  border-radius: 0 0 ${globalTokens.RegularRadius.value}px
    ${globalTokens.RegularRadius.value}px;
...
export default function ChannelList({ channelInfor, accessToken, userId }) {
  ...
  useEffect(() => {
    if (!Array.isArray(lectures) && page !== 1) {
      // lectures !== [] 으로 되어있어서 수정했습니다.
      axios
        .get(
          `https://api.itprometheus.net/channels/${userId}/videos?sort=${
            filterState.sortBy.value
        ...
        .then((res) => {
          console.log("lectures", res.data.data); // 삭제
